...
async function run(){
  ...
  if(!adminUser){
    ...
    await db.run('INSERT INTO users (username, password_hash, role) VALUES (?,?,?)', ['admin', password_hash, 'admin'])
  ...
  let quizId
  if(!sample){
    const r = await db.run('INSERT INTO quizzes (title, description, is_public, created_by) VALUES (?,?,?,?)', ['Contoh Kuis', 'Kuis contoh seeded', 1, null])
    quizId = r.lastID
    await db.run('INSERT INTO questions (quiz_id, text, options_json, correct_index) VALUES (?,?,?,?)',
